...
export default function HeroText({ showButton = true }: HeroTextProps) {
    return (
        <div className="container mx-auto text-white">
            ...
            {showButton && <ApplyButton size="small" className="sm:hidden"  />}
            {showButton && <ApplyButton size="medium" className="hidden sm:block lg:hidden" />}
            {showButton && <ApplyButton size="large" className="hidden lg:block" />}
        </div>
    );
}
