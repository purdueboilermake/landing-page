      ...
      {questions.map((faq, index) => (
        <div key={index} className="max-w-full">
          <h2 className="text-black">
            <button
              type="button"
              className={`flex text-xs md:text-md lg:text-xl items-center justify-between p-2 md:p-3 lg:p-5 font-medium font-subtitle border border-b-0 bg-[#E1E5E7] ${index === 0 ? "rounded-t-xl" : index === questions.length - 1 && openIndex !== index ? "rounded-b-xl" : "" } hover:bg-gray-100 gap-3 w-full`}
